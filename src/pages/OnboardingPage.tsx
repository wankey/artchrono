      ...
      case "course":
        return (
          <StepBox title="1/4 · 添加第一个课程" subtitle="比如：「钢琴」">
            <input className="w-full px-3 py-2 border rounded" placeholder="课程名" value={courseName} onChange={e => setCourseName(e.target.value)} />
        ...
        return (
          <StepBox title="1/4 · 设置等级" subtitle="单节课学费">
            <div className="grid grid-cols-2 gap-2">
              <input type="number" min={1} className="px-3 py-2 border rounded" placeholder="等级序号" value={levelNum} onChange={e => setLevelNum(parseInt(e.target.value)||1)} />
              <input className="px-3 py-2 border rounded" placeholder="等级名" value={levelName} onChange={e => setLevelName(e.target.value)} />
              <input className="px-3 py-2 border rounded" placeholder="单节费（元）" value={priceYuan} onChange={e => setPriceYuan(e.target.value)} />
            </div>
      ...
      case "student":
        return (
          <StepBox title="2/4 · 添加第一个学生" subtitle="">
            <div className="grid grid-cols-2 gap-2">
              <input className="px-3 py-2 border rounded" placeholder="学生姓名" value={studentName} onChange={e => setStudentName(e.target.value)} />
              <input className="px-3 py-2 border rounded" placeholder="家长电话" value={parentPhone} onChange={e => setParentPhone(e.target.value)} />
            </div>
          ...
          <StepBox title="3/4 · 报名 & 付款" subtitle="">
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                <input type="number" min={1} className="w-full px-3 py-2 border rounded" value={classesPaid} onChange={e => { setClassesPaid(parseInt(e.target.value)||0); setAmountYuan(((parseFloat(priceYuan)*parseInt(e.target.value))||0).toString()); }} />
              </div>
              <div>
                <label className="text-xs text-gray-500">金额（元）</label>
                <input className="w-full px-3 py-2 border rounded" value={amountYuan} onChange={e => setAmountYuan(e.target.value)} />
              </div>
            </div>
        ...
        return (
          <StepBox title="4/4 · 排第一节课" subtitle="确定固定上课时间">
            <div className="grid grid-cols-3 gap-2">
              ...
              <input type="time" className="px-3 py-2 border rounded" value={startTime} onChange={e => setStartTime(e.target.value)} />
              <input type="time" className="px-3 py-2 border rounded" value={endTime} onChange={e => setEndTime(e.target.value)} />
            </div>
      ...
      case "done":
        return (
          <div className="bg-green-50 rounded-lg p-8 text-center">
            ...
            <button onClick={onComplete} className="bg-green-600 text-white px-6 py-2 rounded font-medium hover:bg-green-700">开始使用</button>
          </div>
        );
    }
  };
...
function Btn({ label, disabled, onClick, loading }: { label: string; disabled: boolean; onClick: () => void; loading: boolean }) {
  return (
    <button onClick={onClick} disabled={disabled || loading}
      className="flex items-center gap-2 bg-blue-600 text-white px-6 py-2 rounded font-medium hover:bg-blue-700 disabled:opacity-50">
